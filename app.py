...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(tempfile.gettempdir(), 'portfolio_analytics.log')),
...
class AdvancedDataFetcher:
    ...
    def fetch_from_yahoo_finance(self, tickers: List[str], start_date: str,
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Normalize index (timezone, duplicates, sorting) to prevent subtle alignment bugs
                try:
                    if getattr(raw_data.index, "tz", None) is not None:
                        raw_data.index = raw_data.index.tz_localize(None)
                except Exception:
                    # If tz handling fails, we continue with raw index
                    pass

                try:
                    raw_data = raw_data[~raw_data.index.duplicated(keep="first")].sort_index()
                except Exception:
                    pass
                ...
                # Process multi-index columns (Yahoo can return either orientation depending on group_by):
                # - group_by="column": ('Close', 'AAPL')  -> field first
                # - group_by="ticker": ('AAPL', 'Close')  -> ticker first
                if isinstance(raw_data.columns, pd.MultiIndex):
                    prices = pd.DataFrame(index=raw_data.index)
                    volumes = pd.DataFrame(index=raw_data.index)

                    def _mi_get(df: pd.DataFrame, ticker: str, field: str):
                        """Safely get a column from a MultiIndex DataFrame in either (ticker, field) or (field, ticker) orientation."""
                    ...
                    for ticker in tickers:
                        # If auto_adjust=True, Yahoo 'Close' is already adjusted.
                        price_fields = ["Close"] if auto_adjust else ["Adj Close", "Close"]

                        s_price = None
                        for fld in price_fields:
                            s_price = _mi_get(raw_data, ticker, fld)
                            if s_price is not None:
                                break

                        s_vol = _mi_get(raw_data, ticker, "Volume")

                        if s_price is not None:
                            # Ensure Series aligned to the raw index (handles edge cases returning arrays/frames)
                            prices[ticker] = pd.Series(np.asarray(s_price), index=raw_data.index)
                        if s_vol is not None:
                            volumes[ticker] = pd.Series(np.asarray(s_vol), index=raw_data.index)

                    # Drop empty columns (tickers Yahoo didn't return / fully missing)
                    prices = prices.dropna(axis=1, how="all")
                    volumes = volumes.dropna(axis=1, how="all")

                    if prices.empty:
                        raise ValueError(
                            "Yahoo Finance returned a MultiIndex DataFrame but no price columns were extracted. "
                            "This can happen when tickers are invalid, delisted, or the returned column layout differs."
                        )
                else:
                    # Single ticker case
    ...
    def _create_synthetic_data(self, tickers: List[str], start_date: str, 
        ...
        # Returns correspond to date_range[1:], so we generate (n_days - 1) observations.
        n_ret = max(n_days - 1, 1)
        ...
        # Create price series (prices include the initial base price at date_range[0])
        base_prices = np.random.uniform(10, 500, n_tickers)

        # Build a price matrix of shape (n_tickers, n_days):
        # - prices[:, 0] is the initial base price
        # - prices[:, 1:] apply cumulative returns for date_range[1:]
        prices = np.zeros((n_tickers, n_days), dtype=float)
        ...
        # (returns_df is indexed by date_range[1:], while volume_df spans date_range)
        for i, ticker in enumerate(tickers):
            price_changes = returns_df[ticker].abs()

            # Build an aligned factor of length n_days (so it matches volumes[:, i])
            vol_factor = np.ones(n_days, dtype=float)
            try:
                vol_factor[1:] = 1.0 + 0.5 * np.asarray(price_changes.values)
            except Exception:
                # If anything weird happens, keep factor = 1 (no adjustment)
                pass
